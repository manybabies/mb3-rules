...
Participants were `r length(unique(cdat$id))` children 8-12 months of age recruited from...
(7 8-month-olds, 2 9-month-olds, 2 10-month-olds, 4 11-month-olds, and 4 12-month-olds).
[An addition X participants were excluded due to failure to complete...]
...
However, this model's fit was singular, indicating that the random effects structure is too complex for the dataset.
...
[which do we want to drop?]
Table 1 shows the effects from the model with 
...
# singular, random effects need pruning

# 
m1 <- lmer(log_looking_time ~ 1 + 
             fam_condition * trial_type +
             age_sc * trial_type +
             trial_sc * trial_type + (trial * trial_type | id), data=test_dat)
...
```{r, fig.width=4.8, fig.height=3.5, caption="Log(looking time) by trial type and trial number with a simulated main effect (size=.3) of trial type, and bootstrapped 95% confidence intervals."}
plot_model(m2, sort.est = T) + theme_bw()
```

```{r log-looking-time-by-age, fig.width=4.5, fig.height=4, caption="Log(looking time) by trial type and trial number with a simulated main effect (size=.3) of trial type, and bootstrapped 95% confidence intervals."}
